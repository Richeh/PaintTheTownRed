...
const FALLBACK_MARKER_COLOUR = '#E7E5E4';
...
export function createTownRedMap(container, { onReady, onError, onStrokeComplete } = {}) {
  ...
  layerPanel.className = 'absolute left-3 top-3 z-20 hidden w-64 rounded-xl border border-stone-200 bg-white/95 p-3 text-sm text-stone-800 shadow-lg backdrop-blur';
  ...
  layerPanel.innerHTML = `
    ...
    <p class="mt-2 text-[11px] leading-4 text-stone-500">Each person’s paint and points share a layer. Visibility is saved only in this browser.</p>
  `;
  ...
  let hiddenCreators = new Set(), creatorProfiles = new globalThis.Map(), profileGeneration = 0;
  ...
  // Marker permissions, layer labels and point colours all depend on the current
  // profile/uid. AuthDock reloads after identity changes, so one session lookup
  // is sufficient for the lifetime of this renderer.
  ...
  // A contributor gets exactly one layer. Marker-only users therefore appear in
  // the layer list even if they have never painted a stroke on this map.
  function creatorIds() {
    return [...new Set([
      ...strokes.map((s) => s?.created_by),
      ...markers.map((m) => m?.created_by),
    ].filter(Boolean))];
  }

  function creatorProfile(id) { return creatorProfiles.get(id) || null; }
  function creatorLabel(id) {
    const name = creatorProfile(id)?.display_name;
    if (id === currentUserId) return name ? `${name} (You)` : 'You';
    return name || `Collaborator ${id.slice(0, 6)}`;
  }
  function creatorColour(id) { return creatorProfile(id)?.marker_colour || FALLBACK_MARKER_COLOUR; }
  function strokeCount(id) { return strokes.filter((s) => s.created_by === id).length; }
  function markerCount(id) { return markers.filter((m) => m.created_by === id).length; }
  ...
  function renderLayerPanel() {
    ...
    for (const creatorId of creators) {
      ...
      checkbox.addEventListener('change', () => {
        checkbox.checked ? hiddenCreators.delete(creatorId) : hiddenCreators.add(creatorId);
      ...
      swatch.className = 'h-3.5 w-3.5 shrink-0 rounded-full border border-black/10';
      swatch.style.backgroundColor = creatorColour(creatorId);

      const text = document.createElement('span');
      text.className = 'min-w-0 flex-1 truncate';
      text.textContent = creatorLabel(creatorId);
      text.title = creatorProfile(creatorId)?.display_name || creatorId;

      const count = document.createElement('span');
      count.className = 'shrink-0 text-[10px] tabular-nums text-stone-400';
      count.textContent = `${strokeCount(creatorId)}S · ${markerCount(creatorId)}P`;
      count.title = `${strokeCount(creatorId)} strokes · ${markerCount(creatorId)} points`;
  ...
  // Profile requests can resolve out of order when maps change quickly. Keep a
  // generation counter so stale lookups cannot recolour a newer map.
  async function refreshCreatorProfiles() {
    const generation = ++profileGeneration;
    const creators = creatorIds();
    if (!creators.length) {
      creatorProfiles = new globalThis.Map();
      renderLayerPanel();
      renderMarkers();
      return;
    }
    try {
      ...
      creatorProfiles = new globalThis.Map((profiles || []).map((profile) => [profile.user_id, profile]));
      renderLayerPanel();
      renderMarkers();
    } catch (error) {
      if (!destroyed) console.warn('[Town Red] could not load collaborator profiles', error);
    }
  }
  ...
  function visibleMarkers() { return markers.filter((m) => !m?.created_by || !hiddenCreators.has(m.created_by)); }
  ...
  // The whole point pill uses its creator's pastel. A dark symbol/text treatment
  // keeps contrast predictable across the intentionally light palette.
  function markerElement(row) {
    const el = document.createElement('button');
    el.type = 'button';
    el.className = 'flex max-w-52 items-center gap-1.5 rounded-full border border-black/15 px-2 py-1 text-xs font-semibold text-stone-900 shadow-md backdrop-blur hover:brightness-95';
    el.style.backgroundColor = creatorColour(row.created_by);
    const shownLabel = displayMarkerLabel(row.label);
    el.title = row.source_url ? shownLabel : (canManageMarker(row) ? `${shownLabel} — click to edit` : shownLabel);

    const symbol = document.createElement('span');
    symbol.className = 'grid h-5 w-5 shrink-0 place-items-center rounded-full bg-black/10 text-sm text-stone-900';
    ...
    el.addEventListener('click', (event) => { event.stopPropagation(); if (row.source_url) openPropertyPopup(row, el); else openEditMarker(row); });
    return el;
  }
  ...
  function renderMarkers() {
    ...
    mapMarkers = [];
    for (const row of visibleMarkers()) {
  ...
  async function switchMarkerMap(mapId) {
    ...
    try {
      ...
      renderLayerPanel();
      renderMarkers();
      await refreshCreatorProfiles();
      unsubscribeMarkers = await subscribeToMarkerChanges(activeMapId, async () => {
        try {
          markers = await loadMarkers(activeMapId);
          renderLayerPanel();
          renderMarkers();
        ...
        if (['CHANNEL_ERROR', 'TIMED_OUT'].includes(status)) console.warn('[Town Red] marker realtime status', status, error);
      });
    } catch (error) {
  ...
  markerDelete.addEventListener('click',async()=>{if(!editingMarker||!canManageMarker(editingMarker))return;if(!window.confirm(`Delete “${editingMarker.label}”?`))return;markerDelete.disabled=true;markerError.classList.add('hidden');try{await deleteMarker(editingMarker.id);markers=markers.filter(m=>m.id!==editingMarker.id);renderLayerPanel();renderMarkers();await refreshCreatorProfiles();editingMarker=null;pendingPoint=null;markerDialog.close();}catch(e){markerError.textContent=e instanceof Error?e.message:String(e);markerError.classList.remove('hidden');}finally{markerDelete.disabled=false;}});
  markerForm.addEventListener('submit',async(event)=>{event.preventDefault();if(!pendingPoint||!activeMapId||!currentUserId)return;const label=markerLabel.value.trim();if(!label)return;markerSave.disabled=true;markerError.classList.add('hidden');try{let saved;if(editingMarker){saved=await updateMarker({id:editingMarker.id,kind:markerKind.value,label,longitude:pendingPoint.longitude,latitude:pendingPoint.latitude});const i=markers.findIndex(m=>m.id===saved.id);if(i>=0)markers[i]=saved;}else{saved=await createMarker({mapId:activeMapId,userId:currentUserId,kind:markerKind.value,label,longitude:pendingPoint.longitude,latitude:pendingPoint.latitude});markers.push(saved);}renderLayerPanel();renderMarkers();await refreshCreatorProfiles();pendingPoint=null;editingMarker=null;movingMarker=null;markerDialog.close();}catch(e){markerError.textContent=e instanceof Error?e.message:String(e);markerError.classList.remove('hidden');}finally{markerSave.disabled=false;}});

  // -------------------------------------------------------------------------
  // Public stroke/editor API used by Vue
  // -------------------------------------------------------------------------
  function setStrokes(next){strokes=Array.isArray(next)?next:[];const nextMapId=strokes.find(s=>s?.map_id)?.map_id||currentSelectedMapId();if(nextMapId&&nextMapId!==activeMapId){loadLayerPreferences(nextMapId);creatorProfiles=new globalThis.Map();switchMarkerMap(nextMapId);}renderLayerPanel();redraw();refreshCreatorProfiles();}
  function upsertStroke(stroke){if(!stroke?.id)return;const i=strokes.findIndex(s=>s.id===stroke.id);if(i>=0)strokes[i]=stroke;else strokes.push(stroke);if(stroke.map_id&&stroke.map_id!==activeMapId)switchMarkerMap(stroke.map_id);renderLayerPanel();redraw();refreshCreatorProfiles();}
  function removeStroke(id){strokes=strokes.filter(s=>s.id!==id);renderLayerPanel();redraw();refreshCreatorProfiles();}

  // Hidden point layers do not influence fit. That makes the layer toggle a
  // genuine visibility control rather than merely a rendering preference.
  function fitToStrokes(){const points=visibleStrokes().flatMap(normalisePoints),shownMarkers=visibleMarkers();if(!points.length&&shownMarkers.length){const f=shownMarkers[0];map.easeTo({center:[Number(f.longitude),Number(f.latitude)],zoom:14});return true;}if(!points.length)return false;let minLng=Infinity,maxLng=-Infinity,minLat=Infinity,maxLat=-Infinity;for(const p of points){const lng=Number(p.lng),lat=Number(p.lat);minLng=Math.min(minLng,lng);maxLng=Math.max(maxLng,lng);minLat=Math.min(minLat,lat);maxLat=Math.max(maxLat,lat);}if(minLng===maxLng&&minLat===maxLat)map.easeTo({center:[minLng,minLat],zoom:15});else map.fitBounds([[minLng,minLat],[maxLng,maxLat]],{padding:80,maxZoom:16,duration:500});return true;}
